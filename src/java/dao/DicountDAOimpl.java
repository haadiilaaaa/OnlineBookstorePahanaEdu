...
import util.IDGenerator; // This now refers to the interface
import util.NextSequentialIDGenerator; // The concrete implementation
import java.sql.*;
...
public class DicountDAOimpl implements DiscountDAO {
    ...
    public DicountDAOimpl(Connection conn, IDGenerator<String> discountIdGenerator) {
        this.conn = conn;
    ...
    public void save(Discount discount) throws Exception {
        ...
        // The IDGenerator is now responsible for generating a unique ID.
        String newId = discountIdGenerator.generate();
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, discount.getId()); // The generated ID is used here
    ...
    // The method to get the max ID number is removed from the DAO
    // as the ID generation logic is now handled by the IDGenerator class.
