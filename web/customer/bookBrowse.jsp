...
<%
    // This part of the code is correct as it reads cart info from the session
    ...
    if (cart != null) {
        for (CartItem it : cart.values()) {
            ...
            cartTotal = cartTotal.add(it.getPrice().multiply(new BigDecimal(it.getQuantity())));
        }
    }
...
<body>
  ...
  <%
      // Corrected code: Read from the request scope, where the servlet places the messages
    ...
    <form class="search-container" method="get" action="BookBrowseServlet">
      <select name="category" class="category-select">
        ...
        <%
            ...
            String selectedCategoryId = (String) request.getAttribute("selectedCategory");
            if (categories != null) {
      ...
      <input type="text" class="search-box" name="keyword" placeholder="Search books by title/author" value="<%= request.getAttribute("searchKeyword") != null ? request.getAttribute("searchKeyword") : "" %>" />
      <input type="number" step="0.01" name="minPrice" class="search-box" placeholder="Min Price" value="<%= request.getAttribute("minPrice") != null ? request.getAttribute("minPrice") : "" %>" style="max-width:100px;" />
      <input type="number" step="0.01" name="maxPrice" class="search-box" placeholder="Max Price" value="<%= request.getAttribute("maxPrice") != null ? request.getAttribute("maxPrice") : "" %>" style="max-width:100px;" />
      ...
      <% for (ItemDTO item : items) { %>
        <div class="book-card">
          ...
          <% if (item.getImageUrl() != null && !item.getImageUrl().isEmpty()) { %>
            <img class="thumb" src="<%= item.getImageUrl() %>" alt="<%= item.getTitle() %>" />
          <% } %>
          <p><strong>Author:</strong> <%= item.getAuthor() %></p>
          <p>
            <% if (item.isHasDiscount()) { %>  <span class="price-old">Rs. <%= item.getPrice() %></span><br/>
              <span class="price-new">Rs. <%= item.getDiscountedPrice() %></span><br/>
              <% if (item.getDiscountLabel() != null && !item.getDiscountLabel().isEmpty()) { %>
                <span class="badge-offer"><%= item.getDiscountLabel() %></span>
              <% } %>
            <% } else { %>
              <strong>Rs. <%= item.getPrice() %></strong>
            <% } %>
          </p>
